...
screen navigation():
    if renpy.get_screen("main_menu"):
        hbox:
            ...
            # if main_menu:
            imagebutton auto "gui/button/标题_开始游戏_%s.png" action Start()
            # else:
            #     textbutton _("历史") action ShowMenu("history")
            #     textbutton _("保存") action ShowMenu("save")

            imagebutton auto "gui/button/标题_读取进度_%s.png" action ShowMenu("load")
            imagebutton auto "gui/button/标题_环境设置_%s.png" action ShowMenu("preferences")
            # temp
            imagebutton auto "gui/button/标题_特别模式_%s.png" action ShowMenu("preferences")
            ...
            if renpy.variant("pc"):
                ...
                imagebutton auto "gui/button/标题_退出游戏_%s.png" action Quit(confirm=True)
...
screen load():
    ...
    use file_slots(_("读取游戏"),"viewport")

define save_name = ''
screen file_slots(title,scroll=None):
...
style edit_check:
    background "gui/单选框_foreground.png"
    hover_background "gui/单选框_selected_foreground.png"
    foreground "gui/单选框_[prefix_]foreground.png"
    left_padding 50
